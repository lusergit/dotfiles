...
;;
;; Bookmarks: Bookmarks are a way of keeping track of particular
;; comics.  Each comic can have a (default) bookmark, to make it easy
;; to return to a particular date.  Comics can have more than one
;; bookmark, by giving them names.  (The default bookmark corresponds
;; to the name "").  There are global bookmarks, which aren't attached
;; to any comic, but contain the comic information inside.  There is
;; also a default global bookmark.
;;
;; Commands:
...
(defun comics-get-ucomics-dot-com-latest (name abbrev &optional date)
  "Get the latest comic (or DATE) NAME from www.gocomics.com and display ABBREV."
  (if (and
       date
       (not (eq date '-)))
      "onlylatest"
    (comics-get-file-at-url-inside-another-url
     (concat "http://www.gocomics.com/" name)
     (concat
